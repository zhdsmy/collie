...
**Upstream feature conflicts:** the operator chooses whether to retain the downstream feature,
adopt upstream behavior, or combine compatible parts. Before resolving a feature conflict, explain
each option's behavior, user-visible gains, and losses, then wait for the operator's choice. Do not
automatically prefer upstream for overlapping features. Resolve mechanical conflicts and follow
existing explicit decisions (such as disabled workflows) without asking again.
