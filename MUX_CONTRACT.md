...
| `agentDetection` | the `agents`/`shellPanes` split, triage marks | agent name + status + status-change events (**API** § Object shapes, § Event stream) | **no** — only `pane_current_command` / `pane_title` (**T**) | **no** — the listing reports a pane's title and its launch command, and neither is an agent (**Z**) |
