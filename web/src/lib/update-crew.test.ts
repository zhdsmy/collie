...
import { crewAction, crewActionLabel, peerRows, peersBehind, peersRolledBack } from "./update-crew";
import type { UpdateCrewMember, UpdatePeerLeg } from "./types";

// The crew half of the update card, as pure functions. Ordering, counting and the three button
// labels are pinned here rather than by pulling a card apart in the DOM.

function member(over: Partial<UpdateCrewMember> & { name: string }): UpdateCrewMember {
...
describe("peersBehind", () => {
  it("counts only the peers on a version that is not the lead's", () => {
    ...
    expect(peersBehind(crew, "1.4.0")).toBe(1);
...
describe("crewAction", () => {
  it("names the crew when a release is available and this crew has peers", () => {
  ...
  it("names only this machine when there are no peers", () => {
    expect(crewAction({ releaseAvailable: true, hasPeers: false, behind: 0, rolledBack: 0 })).toBe(
      "update",
    );
    expect(crewActionLabel("update", "1.5.0")).toBe("Update to 1.5.0");
  });

  it("offers a retry once the lead is current and a peer is behind or rolled back", () => {
    expect(crewAction({ releaseAvailable: false, hasPeers: true, behind: 1, rolledBack: 0 })).toBe(
      "retry-crew",
    );
    expect(crewAction({ releaseAvailable: false, hasPeers: true, behind: 0, rolledBack: 1 })).toBe(
      "retry-crew",
    );
  ...
  it("offers nothing when the whole crew is level", () => {
    expect(crewAction({ releaseAvailable: false, hasPeers: true, behind: 0, rolledBack: 0 })).toBe("none");
  });
  ...
  it("yields the release branch to the peers when this lead cannot take it", () => {
    expect(
      crewAction({ releaseAvailable: true, hasPeers: true, behind: 1, rolledBack: 0, leadCanTake: false }),
    ).toBe("retry-crew");
    expect(
      crewAction({ releaseAvailable: true, hasPeers: true, behind: 0, rolledBack: 1, leadCanTake: false }),
    ).toBe("retry-crew");
  });

  // THE CONTROL for the pair above: one field differs, and the answer goes back to the button the
  // card disables. Revert the `leadCanTake` branch and the two cases above return `update-crew` —
  // exactly this — while this one keeps passing.
  it("control: the same crew on a lead that CAN take it still names the crew", () => {
    expect(crewAction({ releaseAvailable: true, hasPeers: true, behind: 1, rolledBack: 0 })).toBe("update-crew");
    expect(
      crewAction({ releaseAvailable: true, hasPeers: true, behind: 1, rolledBack: 0, leadCanTake: true }),
    ).toBe("update-crew");
  });
  ...
  it("keeps the release button when there is no peer to yield it to", () => {
    ...
    expect(
      crewAction({ releaseAvailable: true, hasPeers: false, behind: 0, rolledBack: 0, leadCanTake: false }),
    ).toBe("update");
    expect(
      crewAction({ releaseAvailable: true, hasPeers: true, behind: 0, rolledBack: 0, leadCanTake: false }),
    ).toBe("update-crew");
  });
});
