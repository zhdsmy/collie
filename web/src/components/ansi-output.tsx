...
export const AnsiOutput = memo(function AnsiOutput({
      ...
      return (
        <span
          ...
          className={cn(
            ...
            isCurrent
              ? cn(!mirror || rendersNativeMirror(agent, nativeMirror) ? null : MIRROR_INVERT, "bg-yellow-400 text-black")
              : "bg-yellow-400/30",
          )}
        >
          {p.text}
  ...
  const renderLine = (line: StyledLine, li: number, lead: boolean, inRun: boolean): ReactNode => {
    ...
    const content = line.fullWidthRule && wrap && !inRun ? (
      <span className="inline-flex w-full align-bottom whitespace-pre break-normal">
        <span className="w-0 overflow-hidden">{segNodes}</span>
        <span
          aria-hidden="true"
          className="min-w-0 flex-1 self-center border-t"
          style={{ borderColor: line.segments[0] && styleFor(line.segments[0]).color }}
        />
      </span>
    ) : line.fitRule && wrap && !inRun ? (
      <span
  ...
  const rawContent = hasSessionInfo ? rawBlocks.map((block, bi) => {
    ...
    return <pre key={bi} className={preClass(wrap, className, agent, nativeMirror)} style={{ fontSize: `${fontSize}px` }}>
