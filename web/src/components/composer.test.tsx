...
describe("Composer — typing into the terminal", () => {
  ...
  // accessory appearing only after the operator deliberately arms it.
  it("arms the navigation accessory without sending and has no standalone Keys control", () => {
    let replyCalls = 0;
    server.use(replyHandler(() => replyCalls++));
    renderComposer();
    expect(screen.queryByRole("button", { name: /^keys$/i })).not.toBeInTheDocument();
    ...
    expect(screen.getByRole("button", { name: "Show function keys" })).toBeInTheDocument();
  ...
  it("combines modifiers with accessory special keys", async () => {
    ...
    // api.sendKeys expands logical shift+Tab to the PTY's BackTab escape sequence.
    await waitFor(() => expect(keyCalls.flat()).toEqual(["Escape", "[", "Z"]));
  });
...
describe("Composer — display prefs behind the gear", () => {
  ...
  it("the Display dock shares the single drawer slot with Quick", async () => {
    const user = userEvent.setup();
    renderComposer();
  ...
  it("display prefs stay reachable on a read-only device", async () => {
    ...
    // Type/Quick are write affordances and lock; the gear is local view state and must not.
